/**
 * Live integration tests for indent-lang
...
describe.skipIf(!hasWasm)('indent-lang integration (live WASM)', () => {
  // ========== Parser Tests ==========
  ...
  describe('parser', () => {
    ...
    beforeAll(async () => {
      ...
      doc = await createDocumentState(
        wasmPath,
        { uri: 'file:///test.indent', version: 1, languageId: 'indentlang' },
        source,
      );
    });
    ...
    it('loads grammar.wasm and parses test.indent', () => {
    ...
    it('detects parse errors in malformed input', async () => {
      const badDoc = await createDocumentState(
        wasmPath,
        { uri: 'file:///bad.indent', version: 1, languageId: 'indentlang' },
        'def ()',
      );
      try {
  ...
  describe('LSP handlers', () => {
    ...
    it('provides definition for identifier reference', () => {
      // Find a reference to 'x' in the if statement condition
      const ifs = doc.root.descendantsOfType('if_stmt');
    ...
    it('provides references for function declaration', () => {
      ...
      // greet is declared and referenced in the if body
  ...
  describe('cross-file resolution', () => {
    ...
    it('indexes public declarations in workspace', async () => {
      ...
      doc2 = await createDocumentState(
        wasmPath,
        { uri: 'file:///test2.indent', version: 1, languageId: 'indentlang' },
        source2,
      );
